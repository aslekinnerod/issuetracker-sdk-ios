...
private struct TriggerTile: View {
    ...
    private var illustrationView: some View {
        // Resource bundle lookup; falls back to an SF Symbol placeholder
...
//
// `Bundle.module` is auto-generated by Swift Package Manager and only
// exists when the SDK is consumed via SPM. When the Flutter / RN
// wrappers pull this SDK via CocoaPods, that symbol is missing and
// the compile breaks (#error "Type 'Bundle?' has no member 'module'").
//
// Define our own accessor that works in both. CocoaPods packages
// resources declared via `s.resource_bundles` into a sibling .bundle
// next to the framework binary — we find it by class lookup, then
// descend into the bundle's URL.

private final class IssuetrackerResourceBundleToken {}
...
extension Bundle {
    static var sdkResources: Bundle {
        ...
        return .module
        #else
        let containerBundle = Bundle(for: IssuetrackerResourceBundleToken.self)
        if let url = containerBundle.url(forResource: "IssuetrackerSDK", withExtension: "bundle"),
